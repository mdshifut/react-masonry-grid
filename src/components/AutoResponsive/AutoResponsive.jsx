...
class AutoResponsive extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.gridWidth !== nextProps.gridWidth) {
      this.sortManager.changeProps({
  ...
  renderChildren() {
    return React.Children.map(this.props.children, (child, childIndex) => {
      ...
      let childWidth =
        parseInt(child.props.style.width, 10) + this.props.itemMargin;
      let childHeight =
        parseInt(child.props.style.height, 10) + this.props.itemMargin;

      let calculatedPosition = this.sortManager.getPosition(
        childWidth,
        childHeight
      );
